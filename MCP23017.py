...
class MCP23017:
	...
	def setup(self, pin='a0', direction=IN, out_val=LOW, in_polarity=NONINVERT, in_pullup=False, int_en=False, int_on_change=False, int_defval=0):
		...
		read_direction = self.bus.read_byte_data(self.address, self.IODIRA[0] + offset)
		if (direction == self.IN):
			direction_data = read_direction | (1 << int(pin[1]))
			#check and set polarity if input
			read_polarity = self.bus.read_byte_data(self.address, self.IPOLA[0] + offset)
			...
			self.bus.write_byte_data(self.address, self.IPOLA[0] + offset, polarity_data)
			#set or clear pullup
			read_pullup = self.bus.read_byte_data(self.address, self.GPPUA[0] + offset)
			if (in_pullup == True):
			...
			self.bus.write_byte_data(self.address, self.GPPUA[0] + offset, pullup_data)
			#set interupt enable and interrupt type
			read_int_en = self.bus.read_byte_data(self.address, self.GPINTENA[0] + offset)
			if (int_en == True):
				int_en_data = read_int_en | (1 << int(pin[1]))
				read_int_on_change = self.bus.read_byte_data(self.address, self.INTCONA[0] + offset)
				if (int_on_change == True):
					...
					read_defval = self.bus.read_byte_data(self.address, self.DEFVALA[0] + offset)
					...
					self.bus.write_byte_data(self.address, self.DEFVALA[0] + offset, defval_data)
				if (self.debug == True):
					print("Write address 0x{:02x} with data 0x{:02x}".format(self.INTCONA[0] + offset, int_on_change_data))
				self.bus.write_byte_data(self.address, self.INTCONA[0] + offset, int_on_change_data)
			else:
			...
			self.bus.write_byte_data(self.address, self.GPINTENA[0] + offset, int_en_data)
		#check and set output direction
			...
			read_out_val = self.bus.read_byte_data(self.address, self.OLATA[0] + offset)
			...
			self.bus.write_byte_data(self.address, self.OLATA[0] + offset, out_val_data)
		else:
		...
		self.bus.write_byte_data(self.address, self.IODIRA[0] + offset, direction_data)
	...
	def input(self, pin='a0'):
		...
		if ((read_direction >> int(pin[1])) & self.IN != True):
			...
			return -1
		read_gpio = self.bus.read_byte_data(self.address, self.GPIOA[0] + offset)
	...
	def write_regs_por(self):
		for reg in self.list_of_regs:
			if (reg[2] == 'RW'):
				self.bus.write_byte_data(self.address, reg[0],     reg[1])
				self.bus.write_byte_data(self.address, reg[0] + 1, reg[1])
				if (self.debug == True):
					print("Write to {:s}A and {:s}B value 0x{:02x}".format(reg[3],reg[3],reg[1]))

	def read_intcap(self):
		read_intcapa = self.bus.read_byte_data(self.address, self.INTCAPA[0])
		read_intcapb = self.bus.read_byte_data(self.address, self.INTCAPA[0] + 1)
	...
	# Be care when using this function with interrupts enabled as this function will clear the pin interrupt 
	def read_gpio(self):
		read_gpioa = self.bus.read_byte_data(self.address, self.GPIOA[0])
		read_gpiob = self.bus.read_byte_data(self.address, self.GPIOA[0] + 1)
		if (self.debug == True):
